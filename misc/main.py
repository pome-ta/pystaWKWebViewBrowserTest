...
# queue -> マルチスレッド（先入れ先出し）
# weakref -> 弱参照 GC の処理で使う？
# functools -> partial で使ってる
...
class WKWebView(ui.View):
  ...
  def _javascript_alert(self, host, message):
    ...
    # --- 追記 alert console出し
    try:
      my_log = json.loads(message)
      pprint(my_log)
    except :
      print(message)
    #print(json.dump(my_log))
...
v = MyWebView(name='DemoWKWebView',
delegate=MyWebViewDelegate(), 
swipe_navigation=True, data_detectors=(WKWebView.PHONE_NUMBER,WKWebView.LINK), frame=r.bounds, flex='WH')
v.load_url(path)
r.add_subview(v)
r.present()
